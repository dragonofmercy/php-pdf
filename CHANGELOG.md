...
- RFC 3161 timestamping of signatures (PAdES-B-T): pass a `Tsa` to `Document::sign()` (`timestamp: Tsa::http($url)`, optional HTTP Basic auth and SHA-256/384/512). The TimeStampToken from the TSA is embedded as an `id-aa-timeStampToken` unsigned attribute inside the signature CMS, providing a trusted signing time. A `TsaClient` interface (default `HttpTsaClient`) is injectable for custom transports and testing.

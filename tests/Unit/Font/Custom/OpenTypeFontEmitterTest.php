...
use DragonOfMercy\PhpPdf\Font\Custom\FontStream;
...
final class OpenTypeFontEmitterTest extends TestCase
{
    ...
    {
        $result = (new OpenTypeFontEmitter())->emit($this->otf(), $this->subset(), 10, 11, 12, 13, 14);
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), $this->subset(), 10, 11, 12, 13, 14)['cidFont']->toBytes();
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), $this->subset(), 10, 11, 12, 13, 14)['descriptor']->toBytes();
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), $this->subset(), 10, 11, 12, 13, 14)['fontFile']->toBytes();
    ...
    {
        $subsetBytes = str_repeat("\x07", 128);
        $subset = new SubsettedFont($subsetBytes, 'ABCDEF+IBMPlexSans');
        $result = (new OpenTypeFontEmitter())->emit($this->otf(), $subset, 10, 11, 12, 13, 14);
        $bytes = $result['fontFile']->toBytes();
        self::assertStringContainsString('/Subtype /OpenType', $bytes);
        self::assertStringContainsString('/Filter /FlateDecode', $bytes);
        self::assertStringNotContainsString('/Length1', $bytes);
        // Body content: inflate the FlateDecode stream and verify it equals the input.
        $payload = $result['fontFile']->payload();
        self::assertInstanceOf(FontStream::class, $payload);
        $inflated = gzuncompress($payload->body());
        self::assertSame($subsetBytes, $inflated);
    }

    public function testType0ReferencesCidFontAndToUnicode(): void
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), $this->subset(), 10, 11, 12, 13, 14)['type0']->toBytes();

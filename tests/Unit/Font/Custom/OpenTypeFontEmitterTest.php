...
final class OpenTypeFontEmitterTest extends TestCase
{
    ...
    {
        $result = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14);
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14)['cidFont']->toBytes();
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14)['descriptor']->toBytes();
    ...
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14)['fontFile']->toBytes();
    ...
    public function testBaseFontHasNoSubsetPrefix(): void
    {
        $result = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14);
        self::assertStringContainsString('/BaseFont /IBMPlexSans', $result['type0']->toBytes());
        self::assertStringContainsString('/BaseFont /IBMPlexSans', $result['cidFont']->toBytes());
        self::assertDoesNotMatchRegularExpression('#/BaseFont /[A-Z]{6}\+#', $result['type0']->toBytes());
    }

    public function testType0ReferencesCidFontAndToUnicode(): void
    {
        $bytes = (new OpenTypeFontEmitter())->emit($this->otf(), 10, 11, 12, 13, 14)['type0']->toBytes();

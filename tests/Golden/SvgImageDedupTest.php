...
final class SvgImageDedupTest extends TestCase
{
    public function testSvgImageDedupMatchesFixtureBytes(): void
    {
        $expected = file_get_contents(__DIR__ . '/fixtures/svg-image-dedup.pdf');
    ...
    public function testSvgImageDedupPassesQpdfCheck(): void
    {
        $qpdf = (new ExecutableFinder())->find('qpdf');
        if ($qpdf === null) {
            self::markTestSkipped('qpdf is not installed; skipping structural validation.');
        }
        $process = new Process([$qpdf, '--check', __DIR__ . '/fixtures/svg-image-dedup.pdf']);
        $process->run();
        self::assertSame(
            0,
            $process->getExitCode(),

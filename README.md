...
- **SVG vector images** - inline `<svg>` or `.svg` file, fully vector (infinite zoom). Shapes, paths (all commands including arcs), transforms, groups, `<use>` references, `viewBox` + `preserveAspectRatio`, solid fills and strokes with opacity, dash patterns, 147 named CSS colors, linear and radial gradients, embedded raster `<image>` (PNG / JPEG data URI), and `<text>` / `<tspan>` rendered as real selectable text with the standard fonts or any registered custom TTF / OTF family (matched by `font-family`, with automatic glyph subsetting), plus `<textPath>` (text laid along a referenced path, with startOffset and text-anchor). Unsupported features (filters) are skipped silently.
...
- DSS / LTV / PAdES-B-LT, incremental update over externally-supplied PDFs, and certifying (author / MDP) signatures - later phases.
...
- filter: `filter="url(#id)"` / `filter: url(#id)` referencing a `<filter>` is rendered via a hybrid raster approach (matching how Batik / Illustrator export filters to PDF): the filtered region is rasterized to an embedded image (DeviceRGB pixel data + DeviceGray `/SMask` alpha channel) while the rest of the document stays vector. Supported primitives: `feGaussianBlur`, `feOffset`, `feFlood`, `feMerge`, `feBlend` (normal / multiply / screen / darken / lighten), `feComposite` (over / in / out / atop / xor / arithmetic), `feColorMatrix` (matrix / saturate / hueRotate / luminanceToAlpha), and `feDropShadow`. `filterUnits` objectBoundingBox (default) and userSpaceOnUse; `primitiveUnits` defaults to userSpaceOnUse (objectBoundingBox for primitive subregion coordinates is not yet applied - such coordinates are treated as userSpaceOnUse); per-primitive subregions honored. `SourceGraphic` / `SourceAlpha` inputs and named `result` chaining. Rasterized content inside a filter: shapes, paths, solid fills, opacity, groups / `<use>`, linear / radial gradients, and PNG `<image>`. Filters operate in linearRGB. `<text>` inside a filtered element is NOT rasterized: it is drawn as sharp, unfiltered vector text on top (a documented approximation). Resolution defaults to ~300 DPI relative to the SVG viewBox units and is tunable via `Document::setSvgFilterResolution(int $dpi)` (capped at 2000 px per side).

Not supported (skipped silently per SVG spec fallback):

- Per-character positioning (`x="10 20 30"` lists), `letter-spacing` / `word-spacing`, `dominant-baseline` / `baseline-shift`, and `xml:space="preserve"`. Characters outside WinAnsi encoding render as `?`.
...
- Filter primitives not yet implemented: `feTile`, `feImage`, `feMorphology`, `feConvolveMatrix`, `feTurbulence`, `feDisplacementMap`, `feDiffuseLighting`, `feSpecularLighting`. Nested `clipPath` / `mask` / `pattern` / `filter` inside a filtered element are ignored. `color-interpolation-filters="sRGB"` is not honored (filters always run in linearRGB). `BackgroundImage` / `BackgroundAlpha` / `FillPaint` / `StrokePaint` filter inputs are treated as transparent, matching browser behavior. JPEG `<image>` inside a filter requires `ext-gd`; without it that image is skipped.
...
### Standalone SVG export (2D matrix barcodes only)

QR Code, DataMatrix, and Aztec Code can be exported as a self-contained SVG string (no PDF involved), useful when the same app shows the code on screen and embeds it in a PDF without integrating a second barcode library:
...
// "<svg xmlns='...' width='200' height='200' viewBox='0 0 N N'>...</svg>"
...
$doc->sign(
    ...
    timestamp: Tsa::http('https://freetsa.org/tsr'), // optional RFC 3161 timestamp
);
$doc->save('signed.pdf');
```
...
- The `field` must name an existing `SignatureField` (visible or invisible). For additional approval signatures after the initial `sign()`, use `$doc->addSignature($cred, ...)` - each call appends an incremental revision so every subsequent signature cryptographically covers all prior ones.
...
- Not yet supported: DSS / LTV, `ETSI.CAdES.detached`, and certifying (author / MDP) signatures. Document timestamps (`/DocTimeStamp`, `ETSI.RFC3161`) are supported via `$doc->addDocumentTimestamp(Tsa::http($url))` as a separate incremental revision.

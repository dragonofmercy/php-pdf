...
- **1D barcode orientation** - all 1D codes (EAN-13, EAN-8, UPC-A, Code 39, Code 93, Code 128, ITF) implement `OrientableBarcode` and can be drawn vertically with `->vertical()` (quarter-turn CCW, bottom of the code ends at the left). `->horizontal()` is the default. In vertical mode `w` stays the code length along the bars and `h` the bar height, so the visual footprint is `h` wide by `w` tall, anchored at `(x, y)`. Example: `$page->barcode(Code128::of('ABC')->vertical(), x: 20, y: 20, w: 70, h: 18);`. 2D codes (QR, Aztec, DataMatrix, PDF417) are not orientable.

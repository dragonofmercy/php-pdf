...
- 🖋️ **Filling AcroForm fields** - inspect and `setField()` text / checkbox / radio / combobox / listbox values; each filled field gets a generated appearance stream (`/AP`). Fields whose `/DA` references a non-Standard-14 simple font (Type1 / TrueType) embedded in the source `/DR` are also supported: the font is reused from the source, not re-embedded. `flattenFields()` then freezes filled forms into static page content (all fields or a named subset); signature and push-button fields are preserved, and encrypted sources are supported. Type0 / composite embedded fonts are not yet supported.

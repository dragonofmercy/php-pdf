...
- Multiple approval signatures per document, DSS / LTV / PAdES-B-LT, and incremental update over externally-supplied PDFs - later phases.
...
- Not yet supported: multiple approval signatures, DSS / LTV, and `ETSI.CAdES.detached`. Document timestamps (`/DocTimeStamp`, `ETSI.RFC3161`) are supported via `$doc->addDocumentTimestamp(Tsa::http($url))` as a separate incremental revision.

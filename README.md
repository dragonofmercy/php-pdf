...
- **Reading existing PDFs (low-level)** - `PdfReader` parses any non-encrypted PDF (classic xref tables and cross-reference streams, incremental revisions, object streams, the common stream filters) and exposes its objects and page tree. This is the foundation for the modification features.
...
- Filling AcroForm fields and signing existing PDFs - in progress, built on `Pdf::open()`. Importing pages of an existing PDF as reusable templates.

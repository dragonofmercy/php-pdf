...
final class PdfAConformanceGuard
{
    ...
    public function verify(
        ...
        bool $hasForbiddenAttachments = false,
    ): void {
        if ($hasEncryption) {
        ...
        if ($hasForbiddenAttachments) {
            throw new PdfException('This PDF/A level forbids embedded files; use a part-3 level (A3B / A3U) or PDF/A-4f to attach files');
        }
        if ($standardFonts !== []) {

...
/**
 * Standalone SVG renderer for 2D matrix barcodes (QR, DataMatrix, Aztec).
 * Outputs a self-contained SVG string (or a base64 data URI) that any
 * browser/viewer can render without PhpPdf.
 *
 * 1D barcodes are NOT supported and throw on render(): use the PDF path
 * (Page::barcode) for those - their human-text layouts and bearer bars are
 * better served by PDF's text metrics than by a hand-written SVG fallback.
 *
 * PDF417 is also NOT supported: the row-stitched bar layout does not survive
 * the simple per-row run-length emission used here. Use the PDF path.
 */
final class SvgBarcodeRenderer
{
    ...
    public function render(Barcode $barcode, int $width, int $height): string
    {
        ...
        if ($barcode instanceof Pdf417) {
            throw new PdfException('SvgBarcodeRenderer does not support PDF417 (row-stitched layout is not faithfully reproduced). Use Page::barcode() for PDF rendering.');
        }
        $encoded = $barcode->encode();
        if ($encoded->kind !== BarcodeKind::MATRIX_2D) {
            throw new PdfException('SvgBarcodeRenderer only supports 2D matrix barcodes (QR, DataMatrix, Aztec). For 1D, use Page::barcode() to render to a PDF.');
        }
        return $this->render2d($encoded, $width, $height);
    }
